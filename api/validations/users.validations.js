...
const createSchema = joi.object({
        firstName:joi.string().uppercase().trim().regex((/^[A-Za-z]+$/)).required(),
        ...
        systemRole:joi.string().valid(...SYSTEM_ROLES_ENUM).required()
});

const updateSchema = joi.object({
    firstName:joi.string().trim().regex((/^[A-Z]+$/)).uppercase(),
        lastName:joi.string().trim().regex((/^[A-Za-z]+$/)),
