...
const assignmentAdd=joi.object({
    employee:joi.string().regex(/^[a-f0-9]+$/).length(24).required()
})

const assignmentUpdate=joi.object({
    employee:joi.string().regex(/^[a-f0-9]+$/).length(24),
      assignedDate:joi.date().iso(),
      returnedDate:joi.date().iso()
})


module.exports = {createAsset,updateAsset,assignmentAdd,assignmentUpdate,param,query};
